...
function MuzzleyClient (options) {
  ...
  this.maintenance = {
    ...
    reconnectionLimit: options.reconnectionLimit || Infinity,
    reconnectionAttempts: (typeof options.reconnectionAttempts !== 'undefined') ? options.reconnectionAttempts : 10
  };
  ...
  this.idlingMonitor.on('message', function (message) {
    self.trigger('debug', { type: 'message', message: message });
  });
  ...
  this.remoteCalls = new RemoteCalls(this.rpcManager);
}
...
MuzzleyClient.prototype.maybeReconnect = function () {
  ...
  this.reconnectTimeout = setTimeout(function () {
    ...
    if (self.reconnectOptions.type === TYPE_USER) {
...
MuzzleyClient.prototype.__connectCommon = function(type, options, callback) {
  ...
  var onopen = function()  {
    ...
    self.rpcManager.setSocket(self._socket);
    self.remoteCalls.setSocket(self._socket);
  ...
  function connect(remainingAttempts) {
    ...
    connectionTimeout = setTimeout(function () {
      ...
      self.trigger('connect_timeout', TIMEOUT);
    ...
    self._socket.onerror = function onerror(error)  {
      clearTimeout(connectionTimeout);
      if (self.reconnecting) {
        self.trigger('reconnect_error', error);
      }
      self.trigger('connect_error', error);
      retryConnect(--remainingAttempts);
    };
...
MuzzleyClient.prototype.connectUser = function(options, callback){
  ...
  var onopen = function()  {
    ...
    self.middlewareFunctions = [];
    self.middlewareFunctions.push(self.idlingMonitor.middleware);
    self.middlewareFunctions.push(hb);
    self.middlewareFunctions.push(self.rpcManager.handleResponse.bind(self.rpcManager));
    self.middlewareFunctions.push(transformControl);
    self.middlewareFunctions.push(setupComponent);
    self.middlewareFunctions.push(activityTerminated);
    self.middlewareFunctions.push(fileShareInvite);
    self.middlewareFunctions.push(receiveFile);
    self.middlewareFunctions.push(sharingEnd);
    self.middlewareFunctions.push(sharingCancel);
    self.middlewareFunctions.push(signalingMessage);

    self.runMiddlewares = compose.apply(this, self.middlewareFunctions);
