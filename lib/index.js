...
muzzMiddleware.prototype.switchPort = function(){
  this.curPortIndex = (this.curPortIndex + 1) % ports.length;
};

muzzMiddleware.prototype.getEndPoint = function(port, host, schema, path){
  var endPoint = schema || this.endPointSchema;
  endPoint += '://';
  endPoint += host || this.endPointHost;
  endPoint += ':';
  endPoint += port || ports[this.curPortIndex];
  endPoint += '/';
