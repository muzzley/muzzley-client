 ...
 * - `connect`: Fired upon a successful connection, only once. Also listen to `reconnect`.
 ...
 * - `debug`: Fired for specific events that are important for debugging.
 * 
  ...
  var onopen = function()  {
    ...
    function init() {
      self.remoteCalls.init(options, function (err, muzzData){
        ...
        if (!options._backwardCompatible && (!self.reconnecting || !self._connectTriggered)) {
          self.trigger('connect');
          self._connectTriggered = true;
        }
...
/**
 * Creates a subscription to a muzzley channel.
 * 
 * @param  {[type]}   options  An object with the following properties
 *                             - namespace: String, always 'iot',
 *                             - payload: An IoT channel specification object. The more of these properties
 *                             are provided the more specific the subscription gets. That is, it's possible
 *                             to subscribe to all messages of a given profile (device type) or just to
 *                             messages of a specific device's component's property (e.g.: the brightness
 *                             of a specific light bulb).
 *                             The following properties are supported:
 *                               - profile: Your IoT profile's identifier (generated at www.muzzley.com).
 *                               Mandatory.
 *                               - channel: The unique identifier (`remoteId`) you assigned to your own device.
 *                               Optional for device integrators.
 *                               - component: A specific component's identifier of your channel.
 *                               (If you have specified a `bulb` component type, this could be something
 *                               like `bulb-xyz`). Optional.
 *                               - property: The property identifier you want to listen to. Optional.
 *                             
 * @param  {Function} callback Optional callback. We recommend listening to returned channel's
 *                             `subscribe` event.
 * @return {object}            A Channel (`models/Channel.js`) object.
 */
...
MuzzleyClient.prototype.disconnect = function (callback) {
  try {
    this.idlingMonitor.stop();
...
MuzzleyClient.prototype.__handleChannelReconnect = function () {
  ...
  if (!remainingChannels) checkComplete();
